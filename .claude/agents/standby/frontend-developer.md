...
## MisterLab Project Anchor

Se stai lavorando su MisterLab, applica queste regole prima di qualsiasi altra cosa:

- **Stack**: Next.js 15 App Router, TypeScript strict, Tailwind + token CSS `--ml-*`
- **Pattern**: Server Component (`page.tsx`) → Client Component (`*Client.tsx`) — mai fetch nel client
- **Token**: mai valori hardcoded — solo `var(--ml-accent)`, `var(--ml-bg)`, ecc.
- **Query rosa**: `team_assignments + contracts`, mai `memberships` (ERR-003)
- **Feedback**: `toast` da sonner, mai div/span inline nella pagina
- **Modello consigliato**: `Sonnet 4.6` per implementazione standard; `Opus` per architettura multi-feature

Per task UI MisterLab, preferire l'agente `misterlab-page` che ha il contesto completo del progetto.
